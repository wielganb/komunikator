...
class MainActivity : AppCompatActivity() {
    ...
    private fun applySafeInsets(root: View, baseLeft: Int, baseTop: Int, baseRight: Int, baseBottom: Int) {
        // Android 15+ (targetSdk 35) uses edge-to-edge. The safe-area callback
        // is intentionally requested only after the root is attached; requesting
        // it before setContentView() can be ignored on some devices.
        ViewCompat.setOnApplyWindowInsetsListener(root) { view, insets ->
            ...
            val safeLeft = maxOf(bars.left, gestures.left)
            val safeTop = maxOf(bars.top, gestures.top)
            val safeRight = maxOf(bars.right, gestures.right)
            val safeBottom = maxOf(bars.bottom, gestures.bottom, ime.bottom)
        ...
        // The root is not attached yet here. Post the request so every screen
        // receives the real system-bar/keyboard insets after setContentView().
